...
DrawingBoard.Control.DrawingMode = DrawingBoard.Control.extend({
	...
	initialize: function() {
		...
		$.each(["pencil", "eraser", "filler"], $.proxy(function(k, value) {
			if (this.opts[value]) {
				this.$el.append('<button aria-label="' + value + '" class="drawing-board-control-drawingmode-' + value + '-button" data-mode="' + value + '"></button>');
			}
		}, this));
...
DrawingBoard.Control.Download = DrawingBoard.Control.extend({
	...
	initialize: function() {
		this.$el.append('<button aria-label="' + value + '" class="drawing-board-control-download-button"></button>');
