...
DrawingBoard.Board.prototype = {
	...
	setColor: function(color) {
		...
		if (!DrawingBoard.Utils.isColor(color))
			return false;
		this.color = color;
		this.$el.find('.drawing-board-control-colors-current').attr('aria-label', `Color Picker: ${color}`)
...
DrawingBoard.Control.Color = DrawingBoard.Control.extend({
	...
	initTemplate: function() {
		var tpl = '<div class="drawing-board-control-inner">' +
			'<button aria-label="Color Picker: rgba(0,0,0,1)" class="drawing-board-control-colors-current" style="background-color: #000000" data-color="#000000"></button>' +
...
DrawingBoard.Control.DrawingMode = DrawingBoard.Control.extend({
	...
	initialize: function() {
		...
		$.each(["pencil", "eraser", "filler"], $.proxy(function(k, value) {
			if (this.opts[value]) {
				this.$el.append('<button  tabindex="0" aria-label="' + value + '" class="drawing-board-control-drawingmode-' + value + '-button" data-mode="' + value + '"></button>');
			}
		}, this));
...
DrawingBoard.Control.Size = DrawingBoard.Control.extend({
	...
	_dropdownTemplate: function() {
		var tpl = '<div class="drawing-board-control-inner" title="{{size}} pixels">' +
			'<button aria-label="Size Options" class="drawing-board-control-size-dropdown-current"><span></span></button>' +
			'<ul role="menu" tabindex="0" class="drawing-board-control-size-dropdown">';
		$.each(this.opts.dropdownValues, function(i, size) {
			tpl += DrawingBoard.Utils.tpl(
				'<li role="option" tabindex="0" aria-label="{{size}} pixels" data-size="{{size}}"><span style="width: {{size}}px; height: {{size}}px; border-radius: {{size}}px;"></span></li>',
				{ size: size }
			);
		});
		tpl += '</ul>';
